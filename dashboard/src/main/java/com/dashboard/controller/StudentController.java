...
public  @ResponseBody String doValidate(@RequestParam(value ="description") String description,@RequestParam(value ="posttosend") String posttosend,@RequestParam(value ="type") String type,HttpSession session) 
{
System.out.println("im in controller");
AskBean a = new AskBean();
a.setDescription(description);
...
//announcement
		...
		public @ResponseBody String addStatus1(@RequestParam("search") String search,HttpSession session){
			...
			result=result+"<h4 style='margin-left:10px;'><a style='text-decoration:none;' href='viewannouncements.html'>Back to search<<</a></h4>";
			int i=0;
			String studentId=(String) session.getAttribute("pId");
			...
			for(AnnouncementBean sub:announcementBeans){
				...
				for(AnnouncementBean a:readAnnouncements){
					...
					if(sub.getAnnouncemntId()==a.getAnnouncemntId()){
			   			count2=1;
			   			result=result+"<p><div style='margin-left:10px;'><span class='glyphicon glyphicon-calendar'></span><font color='black'>"+dt+"</font></div> <h4 style='text-transform:capitalize;'><a id='a1' href='#!b"+i+"'> &nbsp &nbsp &nbsp"+sub.getSubject()+"</a></h4>&nbsp &nbsp &nbsp &nbsp<font color='#388E8E'>created by:"+sub.getUpdatedBy()+"</font></p>";
			   			result=result+"<div class='box' id='b"+i+"'>&nbsp &nbsp &nbsp &nbsp"+sub.getMessage()+"</div>";
				...
				if(count2==0){
					result=result+"<p><div style='margin-left:10px;'><span class='glyphicon glyphicon-calendar'></span><font color='black'>"+dt+"</font></div> <h4 style='text-transform:capitalize;'><a id='a2' href='#!b"+i+"'> &nbsp &nbsp &nbsp"+sub.getSubject()+"</a></h4>&nbsp &nbsp &nbsp &nbsp<font color='#388E8E'>created by:"+sub.getUpdatedBy()+"</font></p>";
		   			result=result+"<div class='box' id='b"+i+"'>&nbsp &nbsp &nbsp &nbsp"+sub.getMessage()+"</div>";
		   			result=result+"<input type='hidden' id='anmtId"+i+"' value="+sub.getAnnouncemntId()+">";
		   			result=result+"<hr/>";
		   			i++;
